...
/// <summary>
/// Helper class containing generic functions used throughout the RPG Framework.
/// </summary>
public static class UnityTools
{
	/// <summary>
	/// Find components in range of a Vector3 point. 
	/// </summary>
	...
	/// <summary>
	/// Find components in range, which have also component K.
	/// </summary>
	/// <typeparam name="T"></typeparam>
	...
	{
		Vector3 random = new Vector3 (position.x + Random.Range (-range, range), position.y, position.z + Random.Range (-range, range)); 
		RaycastHit hit;
	...
	{
		Vector3 euler = new Vector3 (Random.Range (min, max), Random.Range (min, max), Random.Range (min, max));
		euler.Scale (axis);
	...
	public static Coroutine StartCoroutine (IEnumerator routine, bool dontDestroyOnLoad)
	{
		GameObject routineHandlerObject = new GameObject ("Coroutine");
